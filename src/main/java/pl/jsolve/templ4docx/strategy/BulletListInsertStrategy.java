...
public class BulletListInsertStrategy implements InsertStrategy {
  ...
  public BulletListInsertStrategy(InsertStrategyChooser insertStrategyChooser,
  ...
  public void insert(Insert insert, Variable variable) {
    ...
    for (int i = 0; i < bulletListVariable.getVariables().size(); i++) {
      ...
      Insert copiedInsert = prepareInsert(bulletListInsert, copiedParagraph,
          bulletListInsert.getKey(),
          variableToInsert);
  ...
  private Insert prepareInsert(BulletListInsert originalInsert, XWPFParagraph paragraph, Key key,
    ...
    if (variableToInsert instanceof DocumentVariable) {
      return new DocumentInsert(new Key(key.getKey(), VariableType.DOCUMENT),
          paragraph, originalInsert.getCellParent(), originalInsert.getDocumentParent());
    }
